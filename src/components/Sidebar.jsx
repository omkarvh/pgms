...
export default function Sidebar() {
  ...
  useEffect(() => {
    const unsub1 = onSnapshot(collection(db, 'notifications'), snap => {
      setUnreadCount(snap.docs.filter(d => !d.data().read).length)
    })
    const unsub2 = onSnapshot(collection(db, 'deleteRequests'), snap => {
      setPendingDeletes(snap.docs.filter(d => d.data().status === 'pending').length)
    })
  ...
  const getBadge = (item) => {
    ...
    if (item.path === '/delete-requests' && pendingDeletes > 0) return pendingDeletes
    return null
  }
      ...
      <div className="p-4 border-t border-gray-800">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            ...
            <div className="text-xs text-gray-600 font-mono">{role === 'admin' ? 'Owner' : 'Warden'}</div>
          </div>
        </div>
        <button
