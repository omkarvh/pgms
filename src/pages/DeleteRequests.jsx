...
export default function DeleteRequests() {
        ...
        <main className="flex-1 md:ml-56 flex items-center justify-center px-4">
          <div className="bg-gray-900 border border-red-900 rounded-2xl p-8 text-center max-w-sm">
            ...
            <p className="text-gray-500 text-sm font-mono">
              Only admin can view delete requests.
            </p>
          </div>
        </main>
        <BottomNav />
  ...
  const handleApprove = async (request) => {
    try {
      if (request.collectionName === "tenants_edit") {
        // For edit requests just remove the request — admin handles edit directly
        await deleteDoc(doc(db, "deleteRequests", request.id));
        alert(
          `Edit approved for ${request.label}. Please edit the tenant directly from the Tenants page.`,
        );
      } else {
        ...
        {pending.length === 0 ? (
          ...
          <div className="space-y-3">
            {pending.map((request) => (
              <div
                key={request.id}
                className="bg-gray-900 border border-yellow-500/30 rounded-xl p-5"
              >
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-xs font-mono bg-yellow-500/20 text-yellow-400 px-2 py-0.5 rounded-lg uppercase">
                  ...
                  <div className="flex gap-2 flex-shrink-0">
                    ...
                    >
                      {request.collectionName === "tenants_edit"
                        ? "Approve Edit"
                        : "Approve Delete"}
                    </button>
                  </div>
