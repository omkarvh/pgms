      ...
      <main className="flex-1 md:ml-56 p-4 md:p-8 pb-24 md:pb-8">
        ...
        {rooms.length === 0 ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            {rooms.map(room => (
              <div key={room.id} className={`border rounded-xl p-4 cursor-pointer transition-all hover:scale-105 ${statusColors[room.status]}`}>
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => handleDelete(room.id)} className="flex-1 bg-red-500/20 hover:bg-red-500/30 text-red-400 text-xs py-1 rounded-lg transition-all">Delete</button>
                </div>
              </div>
            ))}
          </div>
