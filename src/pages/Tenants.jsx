...
export default function Tenants() {
  ...
  const handleCheckout = async (tenant) => {
    ...
    await sendNotification(
      "info",
      "🚪 Tenant Checked Out",
      `${tenant.name} has checked out from Room ${tenant.roomNumber}`,
    );
  };
      ...
      <main className="flex-1 md:ml-56 p-4 md:p-8 pb-24 md:pb-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-gray-500 font-mono text-xs mt-0.5">
              {activeTenants.length} active · {leftTenants.length} past
            </p>
          </div>
          <button
            onClick={openAdd}
        ...
        {role === "warden" && (
          <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-3 mb-4 text-yellow-400 text-xs font-mono">
            🔑 Warden — you can add tenants and checkout. To edit tenant
            details, send a request to admin.
          </div>
        )}
        ...
        {displayed.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {displayed.map((tenant) => (
              <div
                key={tenant.id}
                className="bg-gray-900 border border-gray-800 rounded-xl p-5 hover:border-indigo-500/50 transition-all"
              >
                ...
                <div className="space-y-1 mb-4">
                  ...
                  {tenant.email && (
                    <p className="text-xs text-gray-500">✉️ {tenant.email}</p>
                  )}
                  ...
                  <div className="flex gap-2">
                    {role === "admin" ? (
                      <button
                        ...
                        className="flex-1 bg-gray-800 hover:bg-gray-700 text-xs py-1.5 rounded-lg transition-all"
                      >
                        Edit
                      </button>
                    ) : (
                      <button
                        onClick={() => {
                          const message = window.prompt(
                            `What needs to be changed for ${tenant.name} (Room ${tenant.roomNumber})?\n\nDescribe the change clearly:`,
                          );
                          if (message && message.trim()) {
                            requestDelete(
                              "tenants_edit",
                              tenant.id,
                              `Edit request — ${tenant.name} (Room ${tenant.roomNumber}): ${message.trim()}`,
                            ).then(() =>
                    ...
                    <button
                      ...
                      className="flex-1 bg-red-500/20 hover:bg-red-500/30 text-red-400 text-xs py-1.5 rounded-lg transition-all"
                    >
                      Checkout
                    </button>
                  </div>
                ...
                {tab === "left" && (
                  <p className="text-xs text-gray-600 font-mono">
                    Left: {tenant.leftDate?.slice(0, 10)}
                  </p>
                )}
              </div>
            ))}
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 px-4">
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-3">
              ...
              >
                ...
                {vacantRooms.map((r) => (
                  <option key={r.id} value={r.id}>
                    Room {r.number} · {pgConfig.currency}
                    {r.monthlyRate}/mo
                  </option>
                ))}
              </select>
              <select
                value={form.rentMode}
                onChange={(e) => setForm({ ...form, rentMode: e.target.value })}
                className="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:border-indigo-500"
              >
