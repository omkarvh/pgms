...
export default function Tenants() {
  ...
  // Rooms with available beds — not fully occupied, not maintenance
  ...
  const handleSave = async () => {
    ...
    if (editTenant) {
      ...
      if (room) {
        ...
        const newStatus = newOccupied >= capacity ? 'occupied' : 'partial'
        await updateDoc(doc(db, "rooms", form.roomId), {
          occupiedBeds: newOccupied,
          status: newOccupied >= capacity ? 'occupied' : 'occupied' // keep as occupied for partial too
        })
      }

      await sendNotification(
        "booking", "🏠 New Tenant Added",
              ...
              <div key={tenant.id} className="bg-gray-900 border border-gray-800 rounded-xl p-5 hover:border-indigo-500/50 transition-all">
                ...
                {tab === "active" && (
                  <div className="flex gap-2">
                    {role === "admin" ? (
                      <button onClick={() => openEdit(tenant)} className="flex-1 bg-gray-800 hover:bg-gray-700 text-xs py-1.5 rounded-lg transition-all">
                        Edit
                      </button>
                    ) : (
                      <button
